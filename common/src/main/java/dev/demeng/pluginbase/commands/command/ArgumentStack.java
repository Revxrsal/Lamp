...
import dev.demeng.pluginbase.commands.core.LinkedArgumentStack;
 ...
 * <p>
 * This class holds extremely similar functionality to a LinkedList, and in most contexts should be
 * safely castable to one.
 * <p>
 * Command handlers may specify how argument stacks are parsed. See {@link ArgumentParser} for more
 * information.
 *
 * @see ArgumentParser
 */
public interface ArgumentStack extends Deque<String>, List<String>, Cloneable {
  ...
  /**
   * Returns (and removes) the string in which might get concatenated with the rest of the arguments
   * if the parameter {@link CommandParameter#consumesAllString() consumes all strings} that follow
   * it.
   *
  ...
  /**
   * Returns this argument stack as an immutable view. This can be therefore passed to any
   * conditions or resolvers without having to worry about being unintentionally modified.
   * <p>
   * Note that this does not create an independent copy, and instead returns a view which does not
   * allow modifications. If this argument stack gets modified from somewhere else, the immutable
  ...
  /**
   * Returns an immutable copy of this stack. This copy will behave independently of the original
   * {@link ArgumentStack}.
   *
