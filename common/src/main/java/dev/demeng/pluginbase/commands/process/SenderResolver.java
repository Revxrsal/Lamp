...
/**
 * Represents a special resolver for parameters that are always first in command methods. These
 * parameters can be treated as referring to the command actor.
 * <p>
 * Registering a custom sender resolver allows using certain types as senders. The custom types must
 * be tested inside {@link #isCustomType(Class)}, and it is recommended to use
 * {@code CustomType.class.isAssignableFrom(type)} to make sure subclasses are respected.
 * <p>
...
public interface SenderResolver {
   ...
   *
   * @param customSenderType The type of the custom sender. This matches the command parameter
   *                         type.
